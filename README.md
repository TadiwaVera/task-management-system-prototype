...
While this prototype uses a simple relational database, the long-term design envisions a hybrid database architecture, combining a relational database (e.g. PostgreSQL) for structured data (tasks, users, permissions) and a document-oriented database (e.g. MongoDB) for unstructured/semi-structured data (comments, context snapshots, metadata). 
